#!/usr/bin/env python3
"""Backfill small/medium WebP thumbnails for existing movie covers."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate movie-cover thumbnails')
    parser.add_argument(
        '--content-dir',
...
def main():
    args = parse_args()
    cover_dir = os.path.join(args.content_dir, 'covers', 'movie-cover')
    ...
    for original_path in originals:
        ...
        if not missing:
            ...
            continue
        try:
            generate_thumbs(original_path, force=args.force, dry_run=args.dry_run)
            created += 1
